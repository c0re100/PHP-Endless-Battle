...
header('Content-Type: text/html; charset=utf-8');
		...
		if ($NowMS['ID'] != "36" && $SysWepB['id'] != "1001" || $NowMS['ID'] != "37" && $SysWepB['id'] != "1001"  || $NowMS['ID'] != "38" && $SysWepB['id'] != "1001" || $NowMS['ID'] != "39" && $SysWepB['id'] != "1001")
		{
			echo "<td width=\"85\"><input type=submit value=裝備此武器 onClick=\"equipdefform.slot_sw.value='wepb';\"></td>";
		}
		else
		{
			echo "<td width=\"85\">無法裝備</td>";
        }*/
		...
		/*if ($GenVal['msuit']) GetMsDetails("$GenVal[msuit]",'NowMS');
		if ($NowMS['ID'] != "36" && $SysWepC['id'] != "1001" || $NowMS['ID'] != "37" && $SysWepC['id'] != "1001"  || $NowMS['ID'] != "38" && $SysWepC['id'] != "1001" || $NowMS['ID'] != "39" && $SysWepC['id'] != "1001")
		{
			echo "<td width=\"85\"><input type=submit value=裝備此武器 onClick=\"equipdefform.slot_sw.value='wepb';\"></td>";
		}
		else
...
if (!$GameVal['wepa'])
$CEqOpt = "AND `equip` != '2'";
$wepsqlsel = ("SELECT * FROM `".$GLOBALS['DBPrefix']."phpeb_sys_wep` WHERE `kind` REGEXP '.*B.*' AND `price` <= '$GenVal[cash]' AND noshow=0 $CEqOpt ORDER BY familyid, price, grade");
