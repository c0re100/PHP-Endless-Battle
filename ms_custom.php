...
GetMsDetails("$Gen[msuit]",'Pl_Ms');
        //Set DataTable
        $sql = ("SELECT `c_point` FROM `".$GLOBALS['DBPrefix']."phpeb_user_tactfactory` WHERE username='". $_SESSION['username'] ."'");
...
echo "</td><td>名稱預覽:<br><span id=mscname>$Pl_Ms[msname]</span><sub>?</sub>";
echo "</td></tr><tr>";
...
if ($secureCustom){
$sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_tactfactory` SET `time` = '$CFU_Time', `c_point`=`c_point`-".intval($Pl_SyEqWep['grade']*10 + $Pl_Ms['needlv']*2)."  WHERE `username` = '$_SESSION[username]' LIMIT 1;");
mysql_query($sql);
}
