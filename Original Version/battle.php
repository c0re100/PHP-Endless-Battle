...
if (($CFU_Time - $Pl_Gen['btltime']) < $Btl_Intv){echo "距離上次攻擊或移動的時間太短了！<br>請在 ".($Btl_Intv-($CFU_Time - $Pl_Gen['btltime']))." 秒後再進行攻擊！";exit;}
...
if ($Pl_Game['status']){echo "<center>修理中，無法出擊。";postFooter();exit;}
if (!$Pl_WepA[0]) {echo "<center>你沒有裝備武器，不能出擊。";postFooter();exit;}
elseif ($Pl_Game['en'] < ($Pl_SyWepA['enc']+$Pl_SyWepD['enc']+$Pl_SyWepE['enc'])) {echo "EN不足，無法出擊。";postFooter();exit;}
switch(mt_rand(1,2)){
