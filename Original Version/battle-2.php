...
{	unset($AtkFortFlag);
	...
	{echo "不能攻擊要塞";postFooter();exit;}$AtkFortFlag='1';
	...
	$Op_Game = Array("username" => '<AttackFort>',"gamename" => '防禦要塞',"hp" => $Area["User"]["hp"],
	...
	if ($Op_Game['status']){echo "對手修理中．．";postFooter();exit;}
	if ($Pl_Game['status']){echo "修理中，無法出擊。";postFooter();exit;}

	if ($Op_Gen['coordinates'] != $Pl_Gen['coordinates']){echo "看不見對手．．<br>對方位置: $Op_Gen[coordinates] 自己位置: $Pl_Gen[coordinates]";postFooter();exit;}
	...
	if (($Pl_Gen['hypermode'] == 1 || $Pl_Gen['hypermode'] == 5) && $ControlSEED) $SP_Cost = ceil($SP_Cost * 1.25) + 5;	//SEED Mode SP額外消耗
	if ($Pl_Gen['hypermode'] >= 4 && $Pl_Gen['hypermode'] <= 6) $SP_Cost = ceil($SP_Cost * 1.20) + 3;			//EXAM System SP額外消耗
	$SP_CostOP = 0;
	...
	if ($Pl_CFix[0]) $Pl_Ms['msname'] = $Pl_CFix[0]."<sub>&copy;</sub>";
	$Pl_Ms['atf']+=$Pl_CFix[1];$Pl_Ms['def']+=$Pl_CFix[2];
	$Pl_Ms['ref']+=$Pl_CFix[3];$Pl_Ms['taf']+=$Pl_CFix[4];

	GetWeaponDetails("$Pl_WepA[0]",'Pl_SyWepA');
	if ($Pl_WepA[2]){
	if ($Pl_WepA[2]==1) $Pl_SyWepA['name'] = $Pl_WepA[3].$Pl_SyWepA['name']."<sub>&copy;</sub>";
	else $Pl_SyWepA['name'] = $Pl_SyWepA['name'].$Pl_WepA[3]."<sub>&copy;</sub>";
	...
	if ($Pl_WepD[2]){
	if ($Pl_WepD[2]==1) $Pl_SyWepD['name'] = $Pl_WepD[3].$Pl_SyWepD['name']."<sub>&copy;</sub>";
	else $Pl_SyWepD['name'] = $Pl_SyWepD['name'].$Pl_WepD[3]."<sub>&copy;</sub>";
	...
	if ($Pl_WepE[2]){
	if ($Pl_WepE[2]==1) $Pl_SyWepE['name'] = $Pl_WepE[3].$Pl_SyWepE['name']."<sub>&copy;</sub>";
	else $Pl_SyWepE['name'] = $Pl_SyWepE['name'].$Pl_WepE[3]."<sub>&copy;</sub>";
	...
	if ($Op_WepA[2]){
	if ($Op_WepA[2]==1) $Op_SyWepA['name'] = $Op_WepA[3].$Op_SyWepA['name']."<sub>&copy;</sub>";
	else $Op_SyWepA['name'] = $Op_SyWepA['name'].$Op_WepA[3]."<sub>&copy;</sub>";
	...
	if ($Op_WepD[2]){
	if ($Op_WepD[2]==1) $Op_SyWepD['name'] = $Op_WepD[3].$Op_SyWepD['name']."<sub>&copy;</sub>";
	else $Op_SyWepD['name'] = $Op_SyWepD['name'].$Op_WepD[3]."<sub>&copy;</sub>";
	...
	if ($Op_WepE[2]){
	if ($Op_WepE[2]==1) $Op_SyWepE['name'] = $Op_WepE[3].$Op_SyWepE['name']."<sub>&copy;</sub>";
	else $Op_SyWepE['name'] = $Op_SyWepE['name'].$Op_WepE[3]."<sub>&copy;</sub>";
	...
	if (!$Pl_WepA[0]) {echo "你沒有裝備武器，不能出擊。";postFooter();exit;}
	elseif ($Pl_Game['en'] < ($Pl_SyWepA['enc']+$Pl_SyWepD['enc'])) {echo "EN不足，無法出擊。";postFooter();exit;}
	elseif ($Pl_Game['sp'] < $SP_Cost) {echo "SP不足，無法以 $Pl_Tactics[name] 出擊。";postFooter();exit;}
	elseif (!$Pl_Tactics['name']) {echo "不明的戰術。";postFooter();exit;}
	//Specs - Type Customs
	...
	echo "<td bgcolor=$Pl_Gen[color] width=50%><font face=\"Comic Sans MS\" style=\"font-size: 24pt;\"><b>$Pl_Game[gamename]</b></font></td>";
	echo "<td bgcolor=$Op_Gen[color] width=50%><font face=\"Comic Sans MS\" style=\"font-size: 24pt;\"><b>$Op_Game[gamename]</b></font></td>";
	echo "</tr>";
	...
	//Get Hit and Miss Values
		...
		//Specs
			...
			//Type Lower-Case Specs of Miss Values
				...
				if(ereg('(AntiMobS)+',$Pl_Spec) && mt_rand(0,100) >= 50) {$Pl_MobSGd=6; $Spec_Event_Tag .= "<Br>你成功干擾對方的行動！";}
				if(ereg('(AntiMobS)+',$Op_Spec) && mt_rand(0,100) >= 50) {$Op_MobSGd=6; $Spec_Event_Tag .= "<Br>對方成功干擾你的行動！";}
			...
			//Type Lower-Case Specs of Hit Values
				...
				if(ereg('(AntiTarS)+',$Pl_Spec) && mt_rand(0,100) >= 50) {$Pl_TarSGd=6; $Spec_Event_Tag .= "<Br>你成功干擾對方的雷達！";}
				if(ereg('(AntiTarS)+',$Op_Spec) && mt_rand(0,100) >= 50) {$Op_TarSGd=6; $Spec_Event_Tag .= "<Br>對方成功干擾你的雷達！";}
	...
	//Get Defence Values
		...
		$Op_DefX2_pc = mt_rand(0,100);
                if(ereg('(DefX)+',$Pl_Spec) || $Pl_Gen['hypermode'] == 3){if($Pl_DefX_pc >= 40) {$Pl_Def += 20; $Spec_Event_Tag .= "<Br>你的底力爆發了！";}}
                if(ereg('(DefX)+',$Pl_Spec) && $Pl_Gen['hypermode'] == 3){if($Pl_DefX2_pc >= 40) {$Pl_Def += 20; $Spec_Event_Tag .= "<Br>你的念動力之底力爆發了！";}}
                if(ereg('(DefX)+',$Op_Spec) || $Op_Gen['hypermode'] == 3){if($Op_DefX_pc >= 40) {$Op_Def += 20; $Spec_Event_Tag .= "<Br>對手的底力爆發了！";}}
                if(ereg('(DefX)+',$Op_Spec) && $Op_Gen['hypermode'] == 3){if($Op_DefX2_pc >= 40) {$Op_Def += 20; $Spec_Event_Tag .= "<Br>對手的念動力之底力爆發了！";}}
	...
	//Get Attacking Values
		...
		if(ereg('(AtkA)+',$Pl_Spec) && $Spec_AtkA_R >= 50){$Pl_Attacking += 20;$Spec_Event_Tag .= "<Br>你進入了興奮狀態！";}
		if(ereg('(AtkA)+',$Op_Spec) && $Spec_AtkA_R2 >= 50){$Op_Attacking += 20;$Spec_Event_Tag .= "<Br>對手進入了興奮狀態！";}
			//SEED Mode
	...
	//Calculate Left HP and EN
		...
		//Special Event Status
			...
			if(ereg('[DamA]{4}',$Pl_SyWepA['spec']) && $S_Event_ENdam_Rand >= 85 && $VictoryFlag!=2){$Op_Resulting_EN=floor($Op_Resulting_EN*0.5);$Spec_Event_Tag .="<br>對手的機體被損壞，剩餘能源下降！";}
			elseif(ereg('[DamA]{4}',$Pl_SyWepA['spec']) && $S_Event_ENdam_Rand >= 95 && $VictoryFlag==2){$Op_Resulting_EN=floor($Op_Resulting_EN*0.5);$Spec_Event_Tag .="<br>對手的機體被損壞，剩餘能源下降！";}
			if(ereg('[DamA]{4}',$Op_SyWepA['spec']) && $S_Event_ENdam_Rand2 >= 85 && $VictoryFlag!=1){$Pl_Resulting_EN=floor($Pl_Resulting_EN*0.5);$Spec_Event_Tag .="<br>你的機體被對手損壞，剩餘能源下降！";}
			elseif(ereg('[DamA]{4}',$Op_SyWepA['spec']) && $S_Event_ENdam_Rand2 >= 90 && $VictoryFlag==1){$Pl_Resulting_EN=floor($Pl_Resulting_EN*0.5);$Spec_Event_Tag .="<br>你的機體被對手損壞，剩餘能源下降！";}
	//Weapon Stats
		//Nil
	...
	//Analyze Results
		...
		$sql = ("INSERT INTO `".$GLOBALS['DBPrefix']."phpeb_game_history` (`time`, `history`) VALUES (UNIX_TIMESTAMP(), '<font color=$Pl_Org[color]>$Pl_Org[name]</font>的<font color=$Pl_Gen[color]>$Pl_Game[gamename]</font>，成功\\攻陷由<font color=$Area_Org[color]>$Area_Org[name]統治下的$Pl_Gen[coordinates]區域</font>！');");
	...
	//Update Player Status
		...
		//Special Event Status
			...
			if(ereg('(DamB)+',$Pl_SyWepA['spec']) && $S_Event_Status_Rand >= 90 && $VictoryFlag==0){$Op_Game['status']='1';$Spec_Event_Tag .="<br>對手的機體被損壞，戰鬥不能！";}
			elseif(ereg('(DamB)+',$Pl_SyWepA['spec']) && $S_Event_Status_Rand >= 98 && $VictoryFlag==2){$Op_Game['status']='1';$Spec_Event_Tag .="<br>對手的機體被損壞，戰鬥不能！";}
			}
			if(ereg('(AntiDam)+',$Pl_Spec))$S_Event_Status_Rand2=0;
			else {
			if(ereg('(DamB)+',$Op_SyWepA['spec']) && $S_Event_Status_Rand2 >= 90 && $VictoryFlag==0){$Pl_Game['status']='1';$Spec_Event_Tag .="<br>你的機體被對手損壞，戰鬥不能，需要修理！";}
			elseif(ereg('(DamB)+',$Op_SyWepA['spec']) && $S_Event_Status_Rand2 >= 95 && $VictoryFlag==1){$Pl_Game['status']='1';$Spec_Event_Tag .="<br>你的機體被對手損壞，戰鬥不能，需要修理！";}
			}
	//Level Up
		...
		if ($Pl_Game['expr'] >= $UserNextLvExp){$Pl_Game['level'] += 1;$Pl_Game['spmax'] += 1;if($Pl_Game['level']%10==0)$Pl_Game['spmax'] += 5;$Pl_Game['expr'] = 0;$Pl_Gen['growth'] = $Pl_Gen['growth'] + $Pl_Stat_Gain;$Result_Tag .="<br>你升了級！<br>獲得 $Pl_Stat_Gain 點成長點數！";$UpD_Pl_level='1';}
		if ($Op_Game['expr'] >= $OppoNextLvExp){$Op_Game['level'] += 1;$Op_Game['spmax'] += 1;if($Op_Game['level']%10==0)$Op_Game['spmax'] += 5;$Op_Game['expr'] = 0;$Op_Gen['growth'] = $Op_Gen['growth'] + $Op_Stat_Gain;$Result_Tag .="<br>對手升了級！";$UpD_Op_level='1';}
	//Type Level Up
		$TypeRank=0;
	...
	echo "<td>武器: $Pl_SyWepA[name] Exp. $Pl_WepA[1]<br>";
	if ($Pl_WepD[0]) echo "輔助裝備: $Pl_SyWepD[name] Exp. $Pl_WepD[1]<br>";
	if ($Pl_WepE[0]) echo "常規裝備: $Pl_SyWepE[name] Exp. $Pl_WepE[1]<br>";
	echo "</td><td>武器: $Op_SyWepA[name] Exp. $Op_WepA[1]<br>";
	if ($Op_WepD[0]) echo "輔助裝備: $Op_SyWepD[name] Exp. $Op_WepD[1]<br>";
	if ($Op_WepE[0]) echo "常規裝備: $Op_SyWepE[name] Exp. $Op_WepE[1]<br>";
	echo "</td></tr>";
	echo "<tr align=center>";
	echo "<td width=50%>";
	...
	if($StrikeRds) echo "你擊中對手 $StrikeRds 次，並造成 $Pl_Dealt 點傷害。</td>";
	elseif(ereg('[MirrorDam]{9}',$Pl_SyWepA['spec']) && $Pl_Dealt)  echo "反射傷害，造成 $Pl_Dealt 點傷害。</td>";
	else echo "你未能擊中對手！</td>";
	echo "<td>$Op_Tactics[name]<br>";
	if($CStrikeRds && !$OpNoENFlag) echo "對手擊中你 $CStrikeRds 次，並造成 $Op_Dealt 點傷害。</td>";
	elseif($OpNoENFlag == '1'){echo "反擊不能。</td>";}
	elseif(ereg('[MirrorDam]{9}',$Op_SyWepA['spec']) && $Op_Dealt)  echo "反射傷害，造成 $Op_Dealt 點傷害。</td>";
	...
	echo "<br>HP: <span id=Pl_Res_Hp>$Pl_Game[hp]</span>/$Pl_Game[hpmax]<br>消耗EN: ".number_format($Pl_SyWepA['enc']+$Pl_SyWepD['enc']+$Pl_SyWepE['enc'])."</td>";
	echo "<td>";
	...
	echo "<br>HP: <span id=Op_Res_Hp>$Op_Game[hp]</span>/$Op_Game[hpmax]<br>消耗EN: ".number_format($Op_SyWepA['enc']+$Op_SyWepD['enc']+$Op_SyWepE['enc'])."</td>";
	echo "</tr>";
	echo "<tr align=center>";
	echo "<td colspan=2 style=\"color: #FFFF00;font-size: 10\"><br><br><br>";
	echo "得到 $Pl_Gain_Exp 點經驗值和武器經驗 $Pl_Gain_Wep_Exp 點。<br>";
	echo "獲得戰利金 $Pl_Gain_Money 元。";
	if ($Gain_BountyFlag)
