...
if ($mode=='Start'){
	...
	echo "<p align=center style=\"font-size: 16pt\">成立組織完成了！<br>閣下的名聱上升10點。<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
elseif($mode == 'Employ'){
	...
	if(!ereg('(\!'.$Pl_Value['USERNAME'].'\, )+',$Pl_Org['request_list'])){$EmployMsg = "該組織沒有邀請您。";$CancelFlag = '1';}
	...
	echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
	echo "<p align=center style=\"font-size: 16pt\">組織邀請信已發出。<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
elseif ($mode == 'LeaveOrg'){
	...
	echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
	echo "<p align=center style=\"font-size: 16pt\">已脫離組織。<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
if ($actionb = 'A'){
	...
	echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
	echo "<p align=center style=\"font-size: 16pt\">組織已被解散。<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
elseif ($mode == 'JoinOrg'){
	...
	echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
	echo "<p align=center style=\"font-size: 16pt\">加入組織完成了！<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
elseif ($mode == 'ModOrg'){
	...
	$query = mysql_query($sql) or die ('無法取得組織資訊, 原因:' . mysql_error() . '<br>');
	if ($license == 0) $LiText = "即日起<b>接受新會員</b>加入而且會員可以<b>自由脫離</b>組織";elseif ($license == 1) $LiText = "即日起<b>接受新會員<b>加入但<b>限制會員自行退出</b>";
	elseif ($license == 2) $LiText = "即日起<b>不再接受新會員</b>加入但會員可以<b>自由脫離</b>組織";elseif ($license == 3) $LiText = "即日起<b>不再接受新會員</b>加入而且<b>限制會員自行退出</b>";
	...
	echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
	echo "<p align=center style=\"font-size: 16pt\">組織設定完成了！<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
elseif ($mode == 'CityAtk'){
	...
	echo "if ($Pl_Org[funds] < price.innerText){alert('組織資金不足！');return false;}";
	...
	echo "於<select name=sttimedelay><option value=6>6<option value=7>7<option value=8>8<option value=9>9<option value=10>10<option value=11>11<option value=12>12<option value=13>13<option value=14>14<option value=15>15<option value=16>16<option value=17>17<option value=18>18</select>小時後";
	echo "向<select name=target>$AtTarPosblty</select> 發動<br>";
	echo "維持<select name=duration onChange=\"changeDuration()\"><option value=1>1<option value=2>2<option value=3>3</select>小時的戰爭";
	...
	if ($Pl_Org['funds'] < ($Org_War_Cost * $duration)){echo "組織資金不足。";postFooter();exit;}
	...
	$HistoryWrite = "<font color=\"$Pl_Org[color]\">$Pl_Org[name]</font> 對 $target 區域宣戰！";
	WriteHistory($HistoryWrite);
	...
	echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
	echo "<p align=center style=\"font-size: 16pt\">戰爭發動了！<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
elseif ($mode == 'TakeCity'){
	...
	if ($Area["Sys"]["occprice"] > $Pl_Org['funds']){echo "組織資金不足！不能佔領區域。";postFooter();exit;}
	...
	echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
	echo "<p align=center style=\"font-size: 16pt\">成功\佔領了此區域！<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
	echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
	echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
