...
echo "<input type=hidden value='none' name=actionc>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<p align=center style=\"font-size: 16pt\">回收完成了！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('tactfactory.php?action=main')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";        
}//End Reclaim and Put
...
if ($ChosenTact){
        ...
        $CastResult = "製造成功了！<br>你成功製造出 <font color=blue>".$TheResultWep['name']."</font> ！";
...
echo "<input type=hidden value='none' name=actionc>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<p align=center style=\"font-size: 16pt\">$CastResult<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('tactfactory.php?action=main')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";        
}
...
<option value=7>七級<option value=8>八級<option value=9>九級<option value=10>十級<option value=11>十一級</select>合成法。<br>
...
echo "<input type=hidden value='none' name=actionc>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<p align=left style=\"font-size: 16pt\">$CastResult<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('tactfactory.php?action=main')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form></tr></td>";
...
$grade = intval($grade);
if ($grade < 0 || $grade > 11){echo "級別出錯!!<br>";PostFooter();exit;}
...
unset($sql);
$sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_tactfactory` SET `directions` = '". $$TheDisTString ."' WHERE `username` = '$_SESSION[username]' LIMIT 1;");
mysql_query($sql);
