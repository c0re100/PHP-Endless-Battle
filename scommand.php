...
//組織指令
        ...
        $SC_Org .= "<input type=\"submit\" value=\"成立國家\" onClick=\"mainform.action='organization.php?action=Start';actionb.value='A';\">";

        if (!$Game['organization'])
        $SC_Org .= "<input type=\"submit\" value=\"加入國家\" onClick=\"mainform.action='organization.php?action=JoinOrg';actionb.value='A';\">";
		...
		$SC_Org .= "<input type=\"submit\" value=\"晉升\" onClick=\"mainform.action='levelup.php?action=Class';actionb.value='A';\">";
...
//個人指令
		...
		if ($Gen['growth'] >= 200)
		$SC_Prsn .= "<input type=\"submit\" value=\"兌換原料\" onClick=\"mainform.action='exchange.php?action=main';actionb.value='A';\">";
...
//列出指令
        ...
        if ($SC_Org){
        echo "<div align=left><b>國家相關指令:</b></div>";
        echo "$SC_Org";}
        if ($SC_Org_Impt)
...
elseif ($mode=='settings' && $actionb == 'A'){
        ...
        echo "<td><input type=text value=$Pl_Settings[filter_lv_min] name='Pl_Set_filter_lv_min' size=2 maxlength=4>至";
        echo "<input type=text value=$Pl_Settings[filter_lv_max] name='Pl_Set_filter_lv_max' size=2 maxlength=4>";
        echo "</td></tr>";
...
foreach($l as $m){
        ...
        if ($j > 1000) $j = 1000;elseif ($j < 0) $j = 0;
        if ($k > 1000) $k = 1000;elseif ($k < 0) $k = 0;
...
elseif ($mode == 'delete_account' && $actionb == 'B'){
		...
		mysql_query ("DELETE FROM `".$GLOBALS['DBPrefix']."phpeb_bbstoebkeys` WHERE `username` = '".$_SESSION['username']."' Limit 1;");
...
elseif ($mode == 'redeemAlloy' && $actionb == 'A'){
        ...
        echo "<tr><td align=left>";
        echo "您可以在這裡用勝利積分兌換 「合金」以購買合金機體 <br>兌換比率: <b>1 比 ".number_format($VPt2AlloyReq)."</b><Br>即是 ".number_format($VPt2AlloyReq)."點 勝利積分 可以換到 1 個合金！<br>";
...
//Get Warehouse Information
        ...
        if ($defineuserc == 0){
                $sql = ("SELECT * FROM `".$GLOBALS['DBPrefix']."phpeb_user_warehouse` WHERE username='". $_SESSION['username'] ."'");
                $query_whr = mysql_query($sql) or die ('<br><center>未能兌換合金<br>原因:' . mysql_error() . '<br>');
        }
...
//Process
        ...
        if ($Countnumwhwp+$amountAlloy > 100){echo "武器庫空間不足。<br>請先清理一下，多謝合作！";exit;}
                ...
                for($i=1;$i<=$amountAlloy;$i++) $Warehouse[1] .="\n".$AlloyID;
                        ...
                        unset($sql);
                        $sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_general_info` SET `Gold` = Gold+$amountAlloy WHERE `username` = '$_SESSION[username]';");
                        mysql_query($sql);unset($sql);
                        $sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_game_info` SET `v_points` = '".$Game['v_points']."' WHERE `username` = '$_SESSION[username]';");
                        mysql_query($sql);
                ...
                echo "<p align=center style=\"font-size: 16pt\">兌換完成！獲得 $amountAlloy 個合金！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
                echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
                echo "</form>";
