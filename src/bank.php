...
if ($mode=='main' && $actionb=='none'){
        ...
        echo "<tr align=center><td colspan=9><b style=\"font-size: 12pt;\">銀行服務列表: </b></td></tr>";
        if($Bank['status']){
...
mysql_query($sql) or die(mysql_error());unset($sql);
        ...
        echo "<input type=hidden value='none' name=actionb>";
        echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
        echo "<p align=center style=\"font-size: 16pt\">完成！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('bank.php?action=main')\"></p>";
        echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
        echo "</form>";
}
...
elseif($mode == 'SafeHouse' && $actionb && $actionc){
        ...
        //Input Data
        unset($sql);
        $sql =        ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_bank` SET `$PlaceSlotOUT` = '".$exch_target."<#>".$price_sell."<#>".$Game[$sellslot]."<#>$PlaceSlotIN' WHERE `username` = '$_SESSION[username]' LIMIT 1;");
        mysql_query($sql) or die ('無法取得遊戲資訊, 原因:' . mysql_error() . '<br>');unset($sql);
        $sql =        ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_bank` SET  `$PlaceSlotIN` = '".$_SESSION['username']."<#>".$price_sell."<#>".$Game[$sellslot]."<#>$PlaceSlotOUT' WHERE `username` = '$exch_target' LIMIT 1;");
        mysql_query($sql) or die ('無法取得遊戲資訊, 原因:' . mysql_error() . '<br>');unset($sql);
        ...
        echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
        echo "<p align=center style=\"font-size: 16pt\">$Message<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('bank.php?action=SafeHouse')\"></p>";
        echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
        echo "</form>";
