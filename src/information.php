...
if ($AreaInf[0]=='D1'){echo "(格陵蘭大陸)";}
if ($AreaInf[0]=='D2'){echo "(設德蘭群島)";}
if ($AreaInf[0]=='D3'){echo "(摩爾曼斯克軍港)";}
if ($AreaInf[0]=='D4'){echo "(休斯頓基地)";}
if ($AreaInf[0]=='D5'){echo "(直布羅陀海峽)";}
if ($AreaInf[0]=='D6'){echo "(帕米爾高原)";}
if ($AreaInf[0]=='D7'){echo "(珊瑚海海域)";}
if ($AreaInf[0]=='D8'){echo "(乞力馬扎羅)";}
if ($AreaInf[0]=='D9'){echo "(東南亞基地)";}
if ($AreaInf[0]=='D10'){echo "(神秘基地1)";}
if ($AreaInf[0]=='D11'){echo "(神秘基地2)";}
if ($AreaInf[0]=='D12'){echo "(神秘基地3)";}
if ($AreaInf[0]=='D13'){echo "(神秘基地4)";}
if ($AreaInf[0]=='A1'){echo "(馮布朗市)";}
if ($AreaInf[0]=='B1'){echo "(阿·巴瓦·庫)";}
if ($AreaInf[0]=='C1'){echo "(所羅門宇宙海)";}
if ($AreaInf[0]=='A2'){echo "(火星基地)";}
if ($AreaInf[0]=='B2'){echo "(SIDE3)";}
if ($AreaInf[0]=='C2'){echo "(茨之園)";}
if ($AreaInf[0]=='A3'){echo "(L3-X18999)";}
if ($AreaInf[0]=='B3'){echo "(邁錫尼帝國)";}
if ($AreaInf[0]=='C3'){echo "(天頂星帝國)";}
if ($AreaInf[0]=='E1'){echo "(進入太空)";}
...
WHERE `gn`.`username` = `gm`.`username` AND `o`.`id` = `gm`.`organization` AND `ms`.`id` = `gn`.`msuit` AND `type`.`id` = `gn`.`typech` AND `gm`.`isnpc`=0 $SQL_Ex_Str
");
$query = mysql_query($sql) or die ('無法取得基本資訊, 原因:' . mysql_error() . '<br>');
...
if ($PlayerInf['rights'] == '2')
$Rank .= "<font style=\"color: yellow;font-weight: Bold;\"> ".$RightsClass['SMajor']."</font>";
}
else $Rank = '';
...
echo "<center>攻擊技巧: ".dualConvert($PlayerInf['attacking'])." (+$Pl_ATF)   防禦技巧: ".dualConvert($PlayerInf['defending']);
echo " (+$Pl_DEF)   反應技巧: ".dualConvert($PlayerInf['reacting'])." (+$Pl_REF)   命中技巧: ".dualConvert($PlayerInf['targeting']);
echo " (+$Pl_TAF) <br>";
switch ($PlayerInf['status']){case 0: $StatusShow="正常"; $StatusColor='#00CD00';break; case 1: $StatusShow="修理中"; $StatusColor='#FF2200';break;}
echo "勝利積分/次數: $PlayerInf[v_points]/$PlayerInf[victory]                     狀態: <font color=$StatusColor>$StatusShow</font>                     所在區域: $PlayerInf[coordinates]<br>";
...
$sqlgen  = ("SELECT `gamename`,`level`,`coordinates`,org.name As oname FROM `".$GLOBALS['DBPrefix']."phpeb_user_game_info` game,`".$GLOBALS['DBPrefix']."phpeb_user_general_info` gen,`".$GLOBALS['DBPrefix']."phpeb_user_organization` org WHERE ($CFU_Time - `lastlogin`) < ".$GLOBALS['Offline_Time']." AND gen.username = game.username AND org.id = organization ORDER BY organization DESC,coordinates,level DESC");
