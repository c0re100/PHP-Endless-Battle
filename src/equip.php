...
GetUsrDetails("$_SESSION[username]",'','GameVal');
...
echo "<p align=center style=\"font-size: 16pt\">裝備完成了！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('equip.php?action=equip')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
        postFooter();
        }
...
echo "<form action=equip.php?action=equip method=post name=frmeq target=Beta>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<p align=center style=\"font-size: 16pt\">購買完成了！$Pos_Flag<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('equip.php?action=equip')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
        postFooter();
        }
...
echo "<form action=equip.php?action=equip method=post name=frmeq target=Beta>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<p align=center style=\"font-size: 16pt\">售出完成了！<br>你得到了資金 $SellP 元。<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('equip.php?action=equip')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
        postFooter();
        }
...
elseif ($mode == 'chms' && ($actionb == 'chms' || !$actionb)){
GetUsrDetails("$_SESSION[username]",'GenVal','GameVal');
        echo "歡迎來到機體生產工場！！！<hr>";
		...
		echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
		echo "<a href='#' onClick=\"act.action='equip.php?action=buyms';act.actionb.value='buyms';act.target='Beta';act.submit();\">以金錢購買</a>｜<a href='#' onClick=\"act.action='equip.php?action=chms';act.actionb.value='chms';act.target='Beta';act.submit();\">以合金購買</a></form>";
...
echo "<td width=\"50\">攻擊修正</td>";
echo "<td width=\"50\">防禦修正</td>";
echo "<td width=\"50\">迴避修正</td>";
echo "<td width=\"50\">命中修正</td>";
echo "<td width=\"50\">生命加成</td>";
echo "<td width=\"55\">生命回複</td>";
echo "<td width=\"50\">能量加成</td>";
echo "<td width=\"55\">能量回複</td>";
echo "<td width=\"30\">要求等級</td>";
echo "<td width=\"85\">價錢(合金)</td>";
echo "</tr>";
$c=0;
$sqlsysms = ("SELECT * FROM `".$GLOBALS['DBPrefix']."phpeb_sys_ms` WHERE id != '0' AND ShopType = '1' AND noshow='0' ORDER BY needlv DESC, price DESC");
...
echo "<form action=equip.php?action=chms method=post name=buymsform><tr align=center valign=bottom><td colspan=12>";
...
echo "if(priceselctd.innerText > $GenVal[cash]){alert('合金不足!!');return false;}";
echo "else{if (confirm('購買'+msnamesel.innerText+'需要 合金'+priceselctd.innerText+'個。\\n確定要購買嗎？') == true){return true;}else{return false;}}";
echo "}</script>";
...
if ($GenVal['msuit']) GetMsDetails("$GenVal[msuit]",'NowMS');
if ($NowMS['ShopType'] == 1 && $NowMS){
...
echo "<form action=equip.php?action=chms method=post name=sellmsform>";
echo "<input type=hidden value='process' name=actionb>";
...
echo "<tr><td>把此機體解體能夠獲得合金 $SellPrice 個。<br>進行解體: <input type=button value='確定' onclick='cfmsell();'></td></tr>";
echo "</form></table>";
}
postFooter();
}
...
$Otp_Area_Sql = ("SELECT `name`,`color`,`opttime`,`optstart` FROM `".$GLOBALS['DBPrefix']."phpeb_user_organization` WHERE `optmissioni` = 'Atk=($GenVal[coordinates])' AND `opttime` > '$CFU_Time' ORDER BY `optstart` ASC LIMIT 1");
...
if ($Otp_A_ITar && $Otp_A_ITar['optstart'] < $CFU_Time){echo "<center>此區域處於戰爭狀態，機體工廠關閉以躲避戰爭！<br>$Otp_TellTime";postFooter();exit;}
...
echo "</tr>";
$c=0;
$sqlsysms = ("SELECT * FROM `".$GLOBALS['DBPrefix']."phpeb_sys_ms` WHERE id != '0' AND id != '874' AND ShopType = '0' AND noshow='0' ORDER BY needlv DESC, price DESC");
$sysms_q =mysql_query("$sqlsysms");
$thequery = $sysms_q;
$allmsnum = mysql_num_rows($sysms_q);
if ($allmsnum>1){
...
if (!$FlagUnable){
$sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_game_info` SET `$evfrom` = '$evto<!>0' WHERE `username` = '$_SESSION[username]' LIMIT 1;");
mysql_query($sql);
echo "<form action=equip.php?action=equip method=post name=frmeq target=Beta>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<p align=center style=\"font-size: 16pt\">改造完成了！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('equip.php?action=equip')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
postFooter();
...
if($BuyMsDVal['ShopType'] <> 0){echo "購買失敗!!<br>";PostFooter();exit;}
...
if ($NowMS['ShopType'] <> 0){echo "賣出失敗!!<br>";PostFooter();exit;}
$SellPrice = Floor($NowMS['price']/2 + $NowMS['price']*0.2);
...
elseif ($mode == 'chms' && $actionb == 'process'){
GetUsrDetails("$_SESSION[username]",'GenVal','GameVal');
if ($actionc != 'sell'){
if ($GenVal['msuit']){echo "已經有機體!!<br>";PostFooter();exit;}
GetMsDetails("$BuyMsDesired",'BuyMsDVal');
if($BuyMsDVal['ShopType'] <> 1){echo "購買失敗!!<br>";PostFooter();exit;}
if ($GenVal['Gold'] - $BuyMsDVal['price'] < 0){echo "合金不足!!<br>";PostFooter();exit;}
if ($GameVal['level'] < $BuyMsDVal['needlv']){echo "等級不足!!<br>";PostFooter();exit;}
$GenVal['Gold'] = $GenVal['Gold'] - $BuyMsDVal['price'];
$GenVal['msuit'] = $BuyMsDVal['id'];
$GameVal['hpmax'] = $GameVal['hpmax'] + $BuyMsDVal['hpfix'];
$GameVal['enmax'] = $GameVal['enmax'] + $BuyMsDVal['enfix'];
$GameVal['hp'] = $GameVal['hp'] + $BuyMsDVal['hpfix'];
...
$cosuit = array('43','44');
if(!ereg('(co)+',$GenVal['typech']) && in_array($BuyMsDVal['id'],$cosuit)){
	echo "此機體只能由Coordinator人種使用！<br>請購買其他機體！";
	PostFooter();
	exit;
}

$ntsuit = array('45','36');
if(!ereg('(nt)+',$GenVal['typech']) && in_array($BuyMsDVal['id'],$ntsuit)){
	echo "此機體只能由New Type人種使用！<br>請購買其他機體！";
	PostFooter();
	exit;
}

$extsuit = array('41','38');
if(!ereg('(ext)+',$GenVal['typech']) && in_array($BuyMsDVal['id'],$extsuit)){
	echo "此機體只能由Extend人種使用！<br>請購買其他機體！";
	PostFooter();
	exit;
}

$psysuit = array('37');
if(!ereg('(psy)+',$GenVal['typech']) && in_array($BuyMsDVal['id'],$psysuit)){
	echo "此機體只能由Extended人種使用！<br>請購買其他機體！";
	PostFooter();
	exit;
}

$enhsuit = array('39');
if(!ereg('(psy)+',$GenVal['typech']) && in_array($BuyMsDVal['id'],$enhsuit)){
	echo "此機體只能由強化人間人種使用！<br>請購買其他機體！";
	PostFooter();
	exit;
}

$natsuit = array('42');
if(!ereg('(nat)+',$GenVal['typech']) && in_array($BuyMsDVal['id'],$natsuit)){
	echo "此機體只能由一般人種使用！<br>請購買其他機體！";
	PostFooter();
	exit;
}
...
$sql .= ("`en` = $GameVal[en] WHERE `username` = '$_SESSION[username]' LIMIT 1;");
mysql_query($sql);
$sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_general_info` SET `Gold` = '$GenVal[Gold]',
`msuit` = $GenVal[msuit] WHERE `username` = '$_SESSION[username]' LIMIT 1;");
mysql_query($sql);
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">購買完成了！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
...
$Pl_WepE = explode('<!>',$GameVal['p_equip']);
GetWeaponDetails("$Pl_WepE[0]",'Pl_SyWepE');
if ($NowMS['ShopType'] <> 1){echo "賣出失敗!!<br>";PostFooter();exit;}
if (!$GenVal['msuit']){echo "你沒有機體!!<br>";PostFooter();exit;}
$SellPrice = Floor($NowMS['price']/2 + $NowMS['price']*0.2);
$GenVal['Gold'] = $GenVal['Gold'] + $SellPrice;
$GameVal['hpmax'] = $GameVal['hpmax'] - $NowMS['hpfix'];
$GameVal['enmax'] = $GameVal['enmax'] - $NowMS['enfix'];
$GenVal['msuit'] = '0';
...
mysql_query($sql);
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">解體完成了！<br>你得到合金 $SellPrice 個。<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
