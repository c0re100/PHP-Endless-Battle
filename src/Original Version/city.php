...
if ($mode=='ModFort'){
	...
	if ($Area["User"]["at"]+5 <= 145) echo "<br>強化要塞攻擊力: <input type=submit value=\"強化5點AT\" onClick=\"return cfmModFort('at');\"> 所需金錢: $". number_format($At_Cost);
	if ($Area["User"]["de"]+5 <= 145) echo "<br>強化要塞防禦力: <input type=submit value=\"強化5點DE\" onClick=\"return cfmModFort('de');\"> 所需金錢: $". number_format($De_Cost);
	if ($Area["User"]["ta"]+5 <= 145) echo "<br>強化要塞命中能力: <input type=submit value=\"強化5點TA\" onClick=\"return cfmModFort('ta');\"> 所需金錢: $". number_format($Ta_Cost);
