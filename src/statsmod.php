...
if ($mode=='addstat' && $actionb){
        switch($actionb){
        case 'at': if ($Game['attacking'] >= 150){echo "能力過高！";exit;}$NextStatPt_At=$Game['attacking']+1;CalcStatReq('AtAdd',"$NextStatPt_At");if ($Gen['growth']-$AtAdd_Stat_Req < 0){echo "點數不足！";exit;}$Gen['growth']-=$AtAdd_Stat_Req;$Game['attacking']=$NextStatPt_At;$ShowCompl="攻擊能力變成 $Game[attacking] 了。";break;
        case 'de': if ($Game['defending'] >= 150){echo "能力過高！";exit;}$NextStatPt_De=$Game['defending']+1;CalcStatReq('DeAdd',"$NextStatPt_De");if ($Gen['growth']-$DeAdd_Stat_Req < 0){echo "點數不足！";exit;}$Gen['growth']-=$DeAdd_Stat_Req;$Game['defending']=$NextStatPt_De;$ShowCompl="防禦能力變成 $Game[defending] 了。";break;
        case 're': if ($Game['reacting'] >= 100){echo "能力過高！";exit;}$NextStatPt_Re=$Game['reacting']+1; CalcStatReq('ReAdd',"$NextStatPt_Re");if ($Gen['growth']-$ReAdd_Stat_Req < 0){echo "點數不足！";exit;}$Gen['growth']-=$ReAdd_Stat_Req;$Game['reacting'] =$NextStatPt_Re;$ShowCompl="迴避能力變成 $Game[reacting] 了。";break;
        case 'ta': if ($Game['targeting'] >= 140){echo "能力過高！";exit;}$NextStatPt_Ta=$Game['targeting']+1;CalcStatReq('TaAdd',"$NextStatPt_Ta");if ($Gen['growth']-$TaAdd_Stat_Req < 0){echo "點數不足！";exit;}$Gen['growth']-=$TaAdd_Stat_Req;$Game['targeting']=$NextStatPt_Ta;$ShowCompl="命中能力變成 $Game[targeting] 了。";break;
        default : echo "未定義操作！";
        }
...
echo "</form>";
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">改造完成了！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\">";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
        postFooter();
}//End MS
...
if ($Otp_A_ITar && $Otp_A_ITar['optstart'] < $CFU_Time){echo "<center>此區域處於戰爭狀態，修理公廠關閉以躲避戰爭！<br>$Otp_TellTime";postFooter();exit;}
        ...
        echo "<tr><td><b>回復HP:</b><br>HP: $Game[hp] / $Game[hpmax]<br>以百分比回復餘下的 <input type=radio name='hp_rep_type' value='pc' OnClick=\"hp_rep_pc_amount.disabled=false;hp_rep_amount.disabled=true;\">: 回復 <select name='hp_rep_pc_amount' disabled onChange=\"ChangePriceHP('pc')\"><option value=0>--選擇--<option value=0.1>10%<option value=0.2>20%<option value=0.3>30%<option value=0.4>40%<option value=0.5>50%<option value=0.6>60%<option value=0.7>70%<option value=0.8>80%<option value=0.9>90%<option value=1.0 selected>100%</select>(<span id=hppcrep>0</span>點)";
        ...
        echo "<tr><td><b>回復EN:</b><br>EN: $Game[en] / $Game[enmax]<br>以百分比回復餘下的 <input type=radio name='en_rep_type' value='pc' OnClick=\"en_rep_pc_amount.disabled=false;en_rep_amount.disabled=true;\">: 回復 <select name='en_rep_pc_amount' disabled onChange=\"ChangePriceEN('pc')\"><option value=0>--選擇--<option value=0.1>10%<option value=0.2>20%<option value=0.3>30%<option value=0.4>40%<option value=0.5>50%<option value=0.6>60%<option value=0.7>70%<option value=0.8>80%<option value=0.9>90%<option value=1.0 selected>100%</select>(<span id=enpcrep>0</span>點)";
...
if ($Otp_A_ITar && $Otp_A_ITar['optstart'] < $CFU_Time){echo "<center>此區域處於戰爭狀態，修理公廠關閉以躲避戰爭！<br>$Otp_TellTime";postFooter();exit;}
        ...
        if ($actionc == 'hprec' && ($Game['hpmax'] != $Game['hp'])){
                ...
                $sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_game_info` SET `hp` = '$Game[hp]' WHERE `username` = '$_SESSION[username]' LIMIT 1;");
                mysql_query($sql) or die (mysql_error());
                $sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_general_info` SET `cash` = '$Gen[cash]' WHERE `username` = '$_SESSION[username]' LIMIT 1;");
                mysql_query($sql) or die (mysql_error());
        ...
        elseif ($actionc == 'enrec' && ($Game['enmax'] != $Game['en'])){
                ...
                $sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_game_info` SET `en` = '$Game[en]' WHERE `username` = '$_SESSION[username]' LIMIT 1;");
                mysql_query($sql) or die (mysql_error());
                $sql = ("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_general_info` SET `cash` = '$Gen[cash]' WHERE `username` = '$_SESSION[username]' LIMIT 1;");
                mysql_query($sql) or die (mysql_error());
...
echo "</form>";
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">修理完成了！<br>$RepMsg<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
postFooter();exit;        
...
elseif ($mode == 'modtypech' && $actionb == 'A'){
		...
		if(!$Gen['modtype']){echo "您已經使用過人種改造了！每人只能使用一次！";postFooter();exit;}
...
elseif ($mode == 'modtypech' && $actionb == 'B'){
        ...
        if($Gen['cash'] < $ModChType_Cost){echo "現金不足!!";exit;}
					...
					$modt = array('nat','enh','ext','psy','nt','co');
                    $Gen['typech'] = str_replace($modt,$Dest_Type,$Gen['typech']);
